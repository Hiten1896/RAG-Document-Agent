...
// Same reasoning as PdfPreview above: this is CSS-hidden above the md:
// breakpoint (the desktop <aside> panel below handles that case instead),
// so a desktop visitor's initial bundle has no reason to include this JSX,
// its own PdfPreview reference, or its handlers. `ssr: false` isn't strictly
// required here (nothing browser-only inside), but keeps it consistent with
// PdfPreview and avoids rendering mobile-only UI during server render.
const MobileFileViewer = dynamic(() => import('./MobileFileViewer'), {
  ssr: false,
});
...
export default function Home() {
  ...
  return (
    ...
    <div className="flex h-dvh bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased overflow-hidden transition-colors duration-200">
      ...
      {/* Mobile File Viewer — see MobileFileViewer.tsx for why this is a
          separately dynamic-imported chunk rather than inline JSX here. */}
      {viewerFile && (
        <MobileFileViewer
